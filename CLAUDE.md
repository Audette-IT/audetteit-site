...
public/                  # the ONLY folder that is served (assets.directory)
  ...
  404.html               # served for unknown URLs with a 404 status (#76); no twin/route/sitemap
...
- **`wrangler.jsonc`:** `assets.directory` is `./public`, so repo files are
  ...
  `"not_found_handling": "404-page"` (#76) makes unknown URLs return
  `public/404.html` with status 404 and the `_headers` security headers
  (checked locally: `/nope`, `/a/b/c` → 404 with CSP). `/404` itself returns
  200, which is harmless because the page is `noindex`.
- **`worker/index.js`:**
...
- **Markdown for every page** (the owner's requirement: "in all deployments
  ...
  - **`404.html` is the only exception** to the 7 places: no twin, route,
    sitemap, `llms.txt` or canonical, plus `noindex`. `check-links.py` knows.
  - **`check-links.py` also enforces page basics** (#77): meta description
    ≤160 characters, exactly one `<h1>`, `alt` on every `<img>`,
    `og:image:alt` (except the 404), `not_found_handling`, and no
    `console.log`/`debugger`/source maps in `public/`.
...
- **Run the QA:** `cd tools && npm install && node site-qa.js`.
  - It covers 6 pages plus a made-up URL (the 404 page, which must return
    status 404) × 390/1366px × light/dark: overflow, CSP errors, JS
...
- The owner reviews **Artifacts**, not code.
  ...
  banner; the 404 page is reachable from the yellow bar). Publish it with the Artifact tool.
